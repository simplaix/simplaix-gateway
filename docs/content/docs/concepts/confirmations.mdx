...
1. A tool call arrives that matches a `require_confirmation` policy
...
5. A confirmer (admin or designated user) sees the confirmation card in the dashboard
...
| `rejected` | Rejected by a human, request is denied |
