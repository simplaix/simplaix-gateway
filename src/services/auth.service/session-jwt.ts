...
export interface LoginJWTClaims {
  userId: string;
  email: string;
...
export async function issueLoginJWT(claims: LoginJWTClaims): Promise<string> {
  const config = getConfig();
  if (!config.jwtSecret) {
    throw new AuthError('JWT_SECRET not configured — cannot issue login tokens', 'CONFIG_ERROR');
  }
